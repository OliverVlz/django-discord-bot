...
# Asumo que tienes estas variables de entorno configuradas
# DISCORD_BOT_TOKEN, GUILD_ID, DEFAULT_CHANNEL_ID, INVITE_TTL_SECONDS

# La instancia de discord.Client se ejecuta en el comando de administración run_discord_bot.py
# Esta vista no debe crear su propio cliente de Discord.
...
def generate_invite_api(request):
    ...
    try:
        ...
        if not all([guild_id, default_channel_id, discord_bot_token]):
            return JsonResponse({'error': 'Faltan variables de entorno de Discord (GUILD_ID, DEFAULT_CHANNEL_ID, DISCORD_BOT_TOKEN).'}, status=500)
        ...
        if not existing_invite:
            ...
            discord_api_url = f"https://discord.com/api/v10/channels/{default_channel_id}/invites"
            headers = {
